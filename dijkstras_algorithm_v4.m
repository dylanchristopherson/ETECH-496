...
vmax = 3; % (m/s): maximum speed
anmax = 3; % (m/s^2): maximum normal acceleration
atmax = 2; % (m/s^2): maximum tangential acceleration
Rmin = (vmax^2)/anmax; % (M): minimum radius turn at maximum speed and maximum normal acceleration

%% Obstacle field
L=50; W=50; % (m): L x W field
L1=30; W1=4; x1=10; y1=30; % (m): Obstacle 1
L2=5; W2=15; x2=25; y2=10; % (m): Obstacle 2

%% Start position
xs=20; ys=15; % (M): Start location (xs,ys)
HeadingS=0; % (radians): Bot heading at start location: it can be 0, pi/2, pi, and 3*pi/2
SpeedLS=2; % (no unit): Bot speed level at start

%% Discretization
nv = 4; % number of speed levels
dxy = Rmin/(nv-1); % (m): space resolution
VL = zeros(1,4); % 1 to nv speed level array
...
xg=30; yg=45; % Goal location (xg,yg)   % 30, 45
ig=xg/dx; jg=yg/dy;
np=PARENT(ig,jg);
ns=NodeID(is,js);
Traj=[xg yg];
