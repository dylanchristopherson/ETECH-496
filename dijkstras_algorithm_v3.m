...
% location to a goal location using Dijkstra's Algorithm. The bot is
% heading in a specific direction and from that position may go straight,
% left, or right. This restricts movement making cost to travel go up
...
HeadingS = 0; % Bot heading at start location
xg=3; yg=9; % Goal location {xg,yg}
...
BOT_STEPS1=[1 1;1 0;1 -1]; % 1 1 = turn 90 deg left
                           % 1 0 = straight
                           % 1 -1 = turn 90 deg right
% BOT_STEPS1: If heading is in +j direction (3:29)            
BOT_STEPS2=[-1 1;0 1;1 1]; % -1 1 = turn 90 deg left
                           % 0 1 = straight
                           % 1 1 = turn 90 deg right
% BOT_STEPS1: If heading is in -i direction (3:29)
BOT_STEPS3=[-1 -1;-1 0;-1 1]; % -1 -1 = turn 90 deg left
                              % -1 0 = straight
                              % -1 1 = turn 90 deg right
% BOT_STEPS1: If heading is in -j direction (3:29)
BOT_STEPS4=[1 -1;0 -1;-1 -1]; % 1 -1 = turn 90 deg left
                              % 0 -1 = straight
                              % -1 -1 = turn 90 deg right
                               
COST_STEPS=[pi*dx/2 dx pi*dx/2]; % length traveled in each step. 90 degree turn is quarter circle
TURNANGLE=[pi/2 0 -pi/2]; % Heading angle turned (radians) in each step
