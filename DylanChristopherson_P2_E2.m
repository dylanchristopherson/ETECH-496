...
veltraj=zeros(Nt,3); % velocity (x,y,z) in global frame 
...
while nt<Nt
    ...
    veltraj(nt,1)=vx1;veltraj(nt,2)=vy1;veltraj(nt,3)=vz1; % x, y, and z velocities in global frame
...
%% plot x,y, and z velocity in global frame (Used calculations from step 2)
figure(4);hold on;
subplot(3,1,1);
plot(timtraj,veltraj(:,1));xlabel('time(s)');ylabel('X velocity (m/s)');
subplot(3,1,2);
plot(timtraj,veltraj(:,2));xlabel('time(s)');ylabel('Y velocity (m/s)');
subplot(3,1,3);
plot(timtraj,veltraj(:,3));xlabel('time (s)');ylabel('Z velocity (m/s)');
